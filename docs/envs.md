...
| `CRYPTO_BROKER_KMS_DIR` | When using KMS | `configs/` in `.env.example` | Directory containing KMS connection configuration files and user-defined custom client implementations. | Absolute path, or path relative to the process working directory |
...
| `OTEL_PREFIX` | No | Empty | Literal prefix added to exported OpenTelemetry metric and span names; include the desired separator | Any prefix supported by the telemetry backend, for example `crypto-broker.` |
